...
describe("EpubLinkPostProcessor", () => {
	...
	it("adds type, section, and search filters to AI reading notes", () => {
		const container = document.createElement("div");
		container.className = "markdown-rendered";
		container.innerHTML = [
			'<div class="weave-epub-ai-reading-note-root" data-source-file="Books/demo.epub"></div>',
			"<h2>范围摘要</h2><p>overview</p>",
			"<h2>按小节精读</h2>",
			"<h3>准备工作</h3><p>setup detail</p>",
			"<h3>操作步骤</h3><p>steps detail</p>",
			"<h2>章节关系</h2><p>relation detail</p>",
		].join("");
		...
		const sectionSelect = toolbar!.querySelector<HTMLSelectElement>(
			".weave-epub-ai-reading-note-filter-section",
		);
		const searchInput = toolbar!.querySelector<HTMLInputElement>(
			".weave-epub-ai-reading-note-filter-search",
		);
		const count = toolbar!.querySelector<HTMLElement>(
		...
		expect(
			Array.from(sectionSelect?.options || []).map(
				(option) => option.textContent,
			),
		).toEqual(["全部小节", "范围摘要", "准备工作", "操作步骤", "章节关系"]);
		expect(count?.textContent).toBe("4 / 4");

		typeSelect!.value = "sections";
		typeSelect!.dispatchEvent(new Event("change"));
		expect(count?.textContent).toBe("2 / 4");
		expect(
			Array.from(
				container.querySelectorAll<HTMLElement>(
					".weave-epub-ai-reading-note-filtered-block:not(.is-hidden)",
				),
			).map((node) => node.textContent),
		).toEqual(["准备工作", "setup detail", "操作步骤", "steps detail"]);

		typeSelect!.value = "";
		searchInput!.value = "relation";
		searchInput!.dispatchEvent(new Event("input"));
		expect(count?.textContent).toBe("1 / 4");
		expect(
			container
				.querySelector<HTMLElement>("h2:nth-of-type(3)")
				?.classList.contains("is-hidden"),
		).toBe(false);
	});
